...
RC Db::drop_table(const char *table_name)
{
  ...
  // remove from opened tables and delete the table object (will close engine and buffer pool)
  Table *table = iter->second;
  opened_tables_.erase(iter);
  delete table;

  // remove meta, data, lob and index files
  error_code ec;
  filesystem::path meta_file = table_meta_file(path_.c_str(), table_name);
  filesystem::remove(meta_file, ec);
  if (ec) {
    LOG_WARN("Failed to remove table meta file. file=%s, errmsg=%s", meta_file.c_str(), ec.message().c_str());
  }

  filesystem::path data_file = table_data_file(path_.c_str(), table_name);
  filesystem::remove(data_file, ec);
  if (ec) {
    LOG_WARN("Failed to remove table data file. file=%s, errmsg=%s", data_file.c_str(), ec.message().c_str());
  }

  filesystem::path lob_file = table_lob_file(path_.c_str(), table_name);
  filesystem::remove(lob_file, ec);
  if (ec) {
    // lob file may not exist, not necessarily a problem
    LOG_TRACE("remove lob file failed or not exists. file=%s, errmsg=%s", lob_file.c_str(), ec.message().c_str());
  }

  // remove index files with pattern <table>-<index>.index
  string prefix = string(table_name) + "-";
  size_t index_suffix_len = strlen(TABLE_INDEX_SUFFIX);
  for (const auto &entry : filesystem::directory_iterator(path_)) {
    string fname = entry.path().filename().string();
    if (fname.rfind(prefix, 0) == 0) {
      if (fname.size() > index_suffix_len && fname.substr(fname.size() - index_suffix_len) == TABLE_INDEX_SUFFIX) {
        filesystem::remove(entry.path(), ec);
        if (ec) {
          LOG_WARN("Failed to remove index file. file=%s, errmsg=%s", entry.path().c_str(), ec.message().c_str());
        }
      }
    }
  }
